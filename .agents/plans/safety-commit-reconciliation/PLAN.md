...
13. `^seed[query] =select[ordered-evidence] — make structural queries truthful`
14. `&[cli,query] =quote[shell-examples] — preserve selector sigils at the argv boundary`
15. `&[lsp,vscode] =distinguish[couple,capsule] — present angle boundaries without relabeling <>`
16. `.[canon,skills] =align[operational-theory] — restore exhibits after executable behavior exists`
17. `.[plans] =rebase[checkpoint-ecology] — restore plan surfaces and refresh dependency truth`
18. `^seed[form-contours] =project[density] — replay reviewed contour and mobility receipts`
19. `&[runtime,plans] =integrate[liminality-bridge] — replay explicit effects and aligned theory`
...
The VS Code partition consumes v1 directly, keeps URIs intact, repairs command contributions, bundles a matching server inside the extension, and verifies an extracted archive without repository-relative source or a `tsx` launch. The Neovim adapter remains a separate partition: it must negotiate the advertised method, remove its regex-derived root and write authority, preserve opaque URIs, and convert a local `file:` URI only at the final navigation boundary. Configurable index and clustering profiles belong above this contract: saved selectors, exclusions, grouping dimensions, ranking receipts, and explicitly opted-in usage signals may alter a view, but not workspace authority or canonical definitions. Any adaptive view profile must expose its feature schema, signal source, update rule or producer, revision, reset/freeze control, and resulting ranking parameters. These parameters remain inspectable workspace data and cannot alter parsing, authority, identity, or canonical semantics.

Editor presentation must also retain the Seed distinction between the `<>` relation operator and the `<…>` Capsule boundary. The next presentation partition replaces stale “capsule / shell” metadata for `<>`, tokenizes the two-character couple operator as one operator while retaining angle-boundary tokens for actual Capsules, and derives display wording from structural kind/form rather than one overloaded surface string. Canon documents that still reduce Capsule to `<>` move only after that executable distinction is tested.
...
The present selector engine is not ready for arbitrary plugin queries. Audit evidence found that `$@_` consumes only `$`, unknown glyphs and trailing forms can be ignored, `*` collides with the collapse operator, textual brace selectors cover only `[]`, `{}`, and `()`, and the matcher for the selector AST's `seq` node currently behaves as disjunction. Meanwhile, Grok's proposed slice syntax and a range-transform proposal both use `..`, so `..` stays reserved for range/slice study. The bounded `query-truth-v1` partition will require full-token consumption, reject unknown glyphs and unterminated literals, give `any` a distinct node, expose every boundary-coupling discriminant, implement programmatic `seq(...)` as adjacent term-slot groups under one parser-owned Sequence or otherwise-unowned Expression, and return programmatic named-capture evidence. The `$..._` selector envelope becomes a validated query marker plus anonymous-placeholder provenance rather than ignored text; that receipt does not yet localize `_` to a coupling port. Core-pattern `_` and `...@rest` retain their existing grammar meaning outside the query dialect. As an explicit experimental-dialect migration, standalone bare `[]`, `{}`, and `()` now select Frame, Body, and Scope nodes directly; attached bare forms such as `^[]` retain the legacy owner projection, and `$![_]` is the explicit operation-plus-Frame form. Text spelling for ordered groups and named captures remains unassigned until the collision/fixity study is complete. The current CLI already applies flat per-file reference selection across roots; a later `spw/workspaceQuery/v1` will add explicit-root, revisioned URI-grouped transport. Ordered groups never cross files implicitly, and a named cross-file graph edge must carry relation kind, endpoints, provenance, and per-file revision.

Seed already distinguishes the `<>` operator coupling from six boundary couplings: Scope `()`, Frame `[]`, Body `{}`, Capsule `<…>`, Stream `<<…>>`, and NRange `((…))`. Query exposure should project those existing discriminants and shared ports; shared coupling structure does not imply shared physical dynamics. Left/right positions, spans, and depth are parser observations, while polarity or “physics” remains named profile interpretation. Programmatic query selectors now provide named captures with explicit participant receipts; only their textual spelling remains unassigned. Core pattern grammar provides `_` and `...@rest`, but a later text selector design may reuse those forms only after fixity and capture locality are explicit. Ordered results are match groups within one source AST; workspace transport groups those results under a URI rather than manufacturing one cross-file sequence. Local Seed evidence embeds AST nodes and ancestor paths for in-process inspection; `spw/workspaceQuery/v1` must project a compact node-type/span/depth/placeholder/capture/slot/coupling DTO and must not serialize those AST objects wholesale.
