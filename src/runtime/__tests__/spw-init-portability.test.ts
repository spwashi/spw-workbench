...
describe('spw init portability', () => {
  ...
  it('ships a minimal consumer scaffold that includes mount metadata', async () => {
    ...
    const readmePath = path.join(runtime.templateRoot, 'base/.spw/README.md')
    const indexPath = path.join(runtime.templateRoot, 'base/.spw/index.spw')
    ...
    expect(readme).toContain('This `.spw` tree is consumer-owned')
    expect(readme).toContain('exclude it from the consumer corpus')
    expect(readme).toContain('spw -- roots')
    expect(readme).toContain('spw -- tree @spw')
    expect(readme).toContain('spw -- select .spw/index.spw')
    expect(index).toContain('@readme: ~"./README.md"')
    expect(index).toContain('readme: @readme')
